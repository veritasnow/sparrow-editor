...
export function createSelectionService({ root }) {
    ...
    function getActiveKeys() {
        ...
        const range = sel.getRangeAt(0);
        const fragment = range.cloneContents(); // 선택된 영역의 복사본(DOM)
        
        // 1. 선택된 파편(fragment) 내부에서 실제 텍스트가 있는 TD들을 찾음
        const cellsWithContent = Array.from(fragment.querySelectorAll('td[id], th[id]')).filter(cell => {
            // 공백 문자(&ZeroWidthSpace;, \u200B)를 제외한 실제 텍스트가 있는지 검사
            const text = cell.textContent.replace(/\u200B/g, '').trim();
            return text.length > 0;
        });

        if (cellsWithContent.length > 0) {
            // 선택된 셀이 여러 개라면 그 ID들을 모두 반환
            return cellsWithContent.map(c => c.id);
        }

        // 2. 만약 fragment 내부에 TD가 없다면 (한 셀 내부에서만 선택 중인 경우)
        // startContainer를 기준으로 가장 가까운 TD를 찾음
        let startNode = range.startContainer;
        if (startNode.nodeType === Node.TEXT_NODE) startNode = startNode.parentElement;
        const cell = startNode.closest('td[id], th[id]');
        
        if (cell) {
            // 선택된 텍스트가 실제로 있는지 한 번 더 확인 (정밀도)
            const selectedText = range.toString().replace(/\u200B/g, '').trim();
            if (selectedText.length === 0 && !range.collapsed) {
                // 선택은 됐는데 텍스트가 없다면(경계선만 잡힘) 무시
                return [lastActiveKey];
            }
            lastActiveKey = cell.id;
            return [cell.id];
        ...
        // 3. 테이블 밖 일반 영역
        const rootContainer = startNode.closest('[contenteditable="true"]');
        if (rootContainer) {
            lastActiveKey = rootContainer.id;
