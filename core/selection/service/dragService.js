...
export function createDragService(defaultRootId) {
    ...
    function _calculateSelectedCells(e, startTD) {
        ...
        if (startTable.contains(currentTD)) {
            const effectiveEndTD = _findDirectCellOfTable(startTable, currentTD);
            return _getGridCellRange(startTable, startTD, effectiveEndTD);
        }

        // CASE 2: 완전히 다른 테이블 간의 드래그 혹은 부모 테이블로의 드래그
        // 두 셀의 공통 조상 테이블을 찾거나, 없으면 에디터 루트를 기준으로 범위를 잡음
        const commonAncestor = _findCommonAncestor(startTD, currentTD);
        
        if (commonAncestor) {
            // 공통 조상이 테이블인 경우 (중첩 테이블 관계)
            if (commonAncestor.classList.contains('se-table')) {
                const effectiveStart = _findDirectCellOfTable(commonAncestor, startTD);
                const effectiveEnd = _findDirectCellOfTable(commonAncestor, currentTD);
                return _getGridCellRange(commonAncestor, effectiveStart, effectiveEnd);
            } 
            
            // 공통 조상이 에디터 루트인 경우 (형제 테이블 간 드래그)
            // 두 테이블 사이의 모든 요소를 훑어 셀을 수집
            return _getCrossTableCells(commonAncestor, startTD, currentTD);
        }
     ...
     */
    function _findCommonAncestor(node1, node2) {
     ...
     */
    function _getCrossTableCells(root, startNode, endNode) {
     ...
     */
    function _findDirectCellOfTable(table, node) {
        let current = node;
